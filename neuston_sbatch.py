...
SBATCH_DDICT = dict(
    ...
    EMAIL=default_email,
    WALLTIME="24:00:00",
    CUDA_MODULES=CUDA101_MODULES,
    CONDA_ENV="ifcbnn",
    GPU_NUM=1,
    CPU_NUM=4,
    MEM_PER_CPU=10240,  # 10GB
    SLURM_LOG_DIR="slurm-logs",
    SLURM_LOG_FILE="%j.%x.out",
    ABS_CWD=default_cwd,
)
...
def main(parser):
    ...
    if not args.dry_run:
        ...
        if resp.returncode == 0:
            ...
            print(
                "SLURM job_id:  "
                + resp.stderr.strip().replace("sbatch: error:", "<sbatch error>")
            )
...
def argparse_sbatch():
    parser = argparse.ArgumentParser(
        description="SLURM SBATCH auto-submitter for neuston_net.py"
    )
    ...
    slurm.add_argument(
        "--job-name",
        metavar="STR",
        help='Job Name that will appear in slurm jobs list. Defaults is "{}"'.format(
            SBATCH_DDICT["JOB_NAME"]
        ),
    )
    slurm.add_argument(
        "--email",
        help='Email address to send slurm notifications to. Your default is "{}"'.format(
            SBATCH_DDICT["EMAIL"]
        ),
    )
    slurm.add_argument(
        "--walltime",
        metavar="HH:MM:SS",
        help='Set Slurm Task max runtime. Default is "{}"'.format(
            SBATCH_DDICT["WALLTIME"]
        ),
    )
    slurm.add_argument(
        "--gpu-num",
        metavar="INT",
        type=int,
        help="Number of GPUs to allocate per task. Default is {}".format(
            SBATCH_DDICT["GPU_NUM"]
        ),
    )
    slurm.add_argument(
        "--cpu-num",
        metavar="INT",
        type=int,
        help="Number of CPUs to allocate per task. Default is {}".format(
            SBATCH_DDICT["CPU_NUM"]
        ),
    )
    slurm.add_argument(
        "--mem-per-cpu",
        metavar="MB",
        type=int,
        help="Memory to allocate per cpu in MB. Default is {}MB".format(
            SBATCH_DDICT["MEM_PER_CPU"]
        ),
    )
    slurm.add_argument(
        "--slurm-log-dir",
    ...
    slurm.add_argument(
        "--ofile",
        default="{OUTDIR}/{PID}.{JOB_NAME}.sbatch",
        help='Save location for generated sbatch file. Defaults to "{OUTDIR}/{PID}.{JOB_NAME}.sbatch"',
    )
    slurm.add_argument(
        "--conda-env",
        default="ifcbnn",
        help='The conda environment to activate for neuston_net.py. Default is "ifcbnn"',
    )
    slurm.add_argument(
        "--dry-run",
        default=False,
        action="store_true",
        help="Create the sbatch script but do not run it",
    )
