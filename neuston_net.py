...
from neuston_callbacks import SaveTestResults, SaveValidationResults
from neuston_data import IfcbBinDataset, ImageDataset, get_trainval_datasets
...
def do_training(args):
    ...
    if not args.result_files:
        args.result_files = [
            "results.mat training_image_basenames training_classes image_basenames input_classes output_scores confusion_matrix counts_perclass f1_perclass f1_weighted f1_macro".split()
        ]
    for result_file in args.result_files:
        svr = SaveValidationResults(
            outdir=args.outdir, outfile=result_file[0], series=result_file[1:]
        )
    ...
    logger = CSVLogger(
        save_dir=os.path.join(args.outdir, "logs"), name="default", version=None
    )
    ...
    # Do Training
    trainer.fit(
        classifier, train_dataloader=training_loader, val_dataloaders=validation_loader
    )
    ...
    if args.args_log:
        src_path = os.path.join(
            logger.experiment.log_dir, logger.experiment.NAME_HPARAMS_FILE
        )
...
def do_run(args):
    ...
    if args.filter:
        if args.filter[0] not in ["IN", "OUT"]:
    ...
    if not args.outfile:
        ...
        if args.src_type == "img":
            args.outfile = ["img_results.json"]
    ...
    for outfile in args.outfile:
        svr = SaveTestResults(
            outdir=args.outdir, outfile=outfile, timestamp=args.cmd_timestamp
        )
    ...
    if args.src_type == "bin":
        ...
        if args.gobig:
            print("Loading Bins", end=" ")
        for i, bin_fileset in enumerate(dd):
            bin_fileset.pid.namespace = (
                os.path.dirname(bin_fileset.fileset.basepath.replace(args.SRC, ""))
                + os.sep
            )
            ...
            if args.filter:  # applying filter
                if (
                    filter_mode == "IN"
                ):  # if bin does NOT match any of the keywords, skip it
                    if not any([k in str(bin_obj) for k in filter_keywords]):
                        continue
                elif (
                    filter_mode == "OUT"
                ):  # if bin matches any of the keywords, skip it
            ...
            if not args.clobber:
                ...
                if all([os.path.isfile(ofile) for ofile in output_files]):
                    print(
                        "{} result-file(s) already exist - skipping this bin".format(
                            bin_obj
                        )
                    )
                    continue

            bin_dataset = IfcbBinDataset(
                bin_fileset, classifier.hparams.resize, classifier.hparams.img_norm
            )
    ...
    ## IMAGES ##
        ...
        if os.path.isdir(args.SRC):
            ...
            IMG_EXTENSIONS
        ):  # single img # TODO TEST: single img run
        ...
        if args.filter:
            for img in img_paths[:]:
                if (
                    filter_mode == "IN"
                ):  # if img does NOT match any of the keywords, skip it
                    if not any([k in img for k in filter_keywords]):
                        img_paths.remove(img)
                elif (
                    filter_mode == "OUT"
                ):  # if img matches any of the keywords, skip it
                    if any([k in img for k in filter_keywords]):
                        img_paths.remove(img)

        assert len(img_paths) > 0, "No images to process"
        image_dataset = ImageDataset(
            img_paths, resize=classifier.hparams.resize, input_src=args.SRC
        )
        image_loader = DataLoader(
            image_dataset,
            batch_size=args.batch_size,
...
def argparse_nn(parser=None):
    ...
    subparsers = parser.add_subparsers(
        dest="cmd_mode",
        help='These sub-commands are mutually exclusive. Note: optional arguments (below) must be specified before "TRAIN" or "RUN"',
    )
    ...
    common.add_argument(
        ...
        default=108,
        type=int,
        help="Number of images per batch. Defaults is 108",
    )  # todo: auto-mode built in to ptl
    common.add_argument(
        "--loaders",
        metavar="N",
        default=4,
        type=int,
        help="Number of data-loading threads. 4 per GPU is typical. Default is 4",
    )  # todo: auto-mode?
...
def argparse_nn_train(train_subparser):
    ...
    train_subparser.add_argument(
        "SRC",
        help="Directory with class-label subfolders and images. May also be a dataset-configuration csv.",
    )
    train_subparser.add_argument(
        "MODEL", help='Select a base model. Eg: "inception_v3"'
    )
    # TODO choices field. TODO: "Accepts a known model name, or a path to a specific model file for transfer learning"
    train_subparser.add_argument(
        "TRAIN_ID",
        help="Training ID. This value is the default value used by --outdir and --model-id.",
    )

    model = train_subparser.add_argument_group(
        title="Model Adjustments", description=None
    )
    model.add_argument(
        "--untrain",
        dest="pretrained",
    ...
    data = train_subparser.add_argument_group(
        title="Dataset Adjustments", description=None
    )
    data.add_argument(
        "--seed",
        default=0,
        type=int,
        help="Set a specific seed for deterministic output & dataset-splitting reproducability.",
    )
    data.add_argument(
        "--split",
        metavar="T:V",
        default="80:20",
        help='Ratio of images per-class to split randomly into Training and Validation datasets. Randomness affected by SEED. Default is "80:20"',
    )
    data.add_argument(
        "--class-config",
        metavar=("CSV", "COL"),
    ...
    data.add_argument(
        "--swap", default=False, action="store_true", help=argparse.SUPPRESS
    )  # dupes placeholder. may not be needed.

    epochs = train_subparser.add_argument_group(
        title="Epoch Parameters", description=None
    )
    epochs.add_argument(
        "--emax",
        metavar="MAX",
        default=60,
        type=int,
        help="Maximum number of training epochs. Default is 60",
    )
    epochs.add_argument(
        "--emin",
        metavar="MIN",
        default=10,
        type=int,
        help="Minimum number of training epochs. Default is 10",
    )
    epochs.add_argument(
        "--estop",
        metavar="STOP",
        default=10,
        type=int,
        help="Early Stopping: Number of epochs following a best-epoch after-which to stop training. Set STOP=0 to disable. Default is 10",
    )

    augs = train_subparser.add_argument_group(
        title="Augmentation Options",
        description="Data Augmentation is a technique by which training results may improved by simulating novel input",
    )
    augs.add_argument(
        "--flip",
        choices=["x", "y", "xy", "x+V", "y+V", "xy+V"],
    ...
    out.add_argument(
        "--outdir",
        default="training-output/{TRAIN_ID}",
        help='Default is "training-output/{TRAIN_ID}"',
    )
    out.add_argument(
        "--model-id",
        default="{TRAIN_ID}",
        help='Set a specific model id. Patterns {TRAIN_DATE} and {TRAIN_ID} are recognized. Default is "{TRAIN_ID}"',
    )
    out.add_argument(
        "--epochs-log",
        metavar="ELOG",
        default="epochs.csv",
        help="Specify a csv filename. Includes epoch, loss, validation loss, and f1 scores. Default is epochs.csv",
    )
    out.add_argument(
        "--args-log",
        metavar="ALOG",
        default="args.yml",
        help="Specify a human-readable yaml filename. Includes all user-specified and default training parameters. Default is args.yml",
    )
    out.add_argument(
        "--onnx",
        action="store_true",
        help="Additionally output an onnx version of the model",
    )
    out.add_argument(
        "--results",
        dest="result_files",
        metavar=("FNAME", "SERIES"),
        nargs="+",
        action="append",
    ...
    meta.add_argument(
        "--dataset-id", help="Associate a dataset id label with this model"
    )
    meta.add_argument(
        "--notes",
        help='Add any kind of note to the trained model. Make sure to use quotes "around your message."',
    )
...
def argparse_nn_run(run_subparser):
    ...
    run_subparser.add_argument(
        "SRC",
        help="Resource(s) to be classified. Accepts a bin, an image, a text-file, or a directory. Directories are accessed recursively",
    )
    ...
    run_subparser.add_argument(
        ...
        default="bin",
        choices=["bin", "img"],
        help='File type to perform classification on. Defaults is "bin"',
    )
    run_subparser.add_argument(
        "--outdir",
        default="run-output/{RUN_ID}/v3/{MODEL_ID}",
        help='Default is "run-output/{RUN_ID}/v3/{MODEL_ID}"',
    )
    run_subparser.add_argument(
        "--outfile",
        action="append",
    ...
    run_subparser.add_argument(
        "--gobig", action="store_true", help=argparse.SUPPRESS
    )  # aggregates bins
...
def proc_outdir(args):
    ...
    if args.cmd_mode == "TRAIN":
        args.outdir = args.outdir.format(
            TRAIN_DATE=run_date_str, TRAIN_ID=args.TRAIN_ID
        )
    elif args.cmd_mode == "RUN":
        model_id = NeustonModel.load_from_checkpoint(args.MODEL).hparams.model_id
        args.outdir = args.outdir.format(
            RUN_DATE=run_date_str, RUN_ID=args.RUN_ID, MODEL_ID=model_id
        )
