...
class NeustonDataset(Dataset):
    def __init__(
        ...
        self.minimum_images_per_class = max(
            1, minimum_images_per_class
        )  # always at least 1.
        images_perclass__minthresh = {
            label: images
            for label, images in images_perclass.items()
            if len(images) >= self.minimum_images_per_class
        }
        classes_ignored = sorted(
            set(images_perclass.keys()) - set(images_perclass__minthresh.keys())
        )
        self.classes_ignored_from_too_few_samples = [
            (c, len(images_perclass[c])) for c in classes_ignored
        ]
        ...
        if maximum_images_per_class:
            ...
            self.classes_limited_from_too_many_samples = [
                ...
                if len(images_perclass__maxlimited[c])
                < len(images_perclass__minthresh[c])
            ]
        else:
        ...
        images_perclass__final = {
            label: sorted(images) for label, images in images_perclass__final.items()
        }

        # flatten images_perclass to congruous list of image paths and target id's
        self.targets, self.images = zip(
            *(
                (self.classes.index(t), i)
                for t in images_perclass__final
                for i in images_perclass__final[t]
            )
        )
    ...
    def fetch_images_perclass(cls, src, include_exclude_rename=None):
            ...
            for subdir in classes:
                ...
                files = sorted(
                    [
                        f
                        for f in files
                        if os.path.splitext(f)[1] in datasets.folder.IMG_EXTENSIONS
                    ]
                )
        ...
        # classes are being adjusted on a per-dataset level
            ...
            for i in range(len(cols)):
                ...
                dataset_images_perclass = cls.fetch_images_perclass(
                    dataset, include_exclude_rename=include_exclude_rename__PARAM
                )

                datasets_by_priority.append(
                    (priority, dataset, dataset_images_perclass)
                )
            ...
            datasets_by_priority = (
                ((max(priorities) if p == 0 else p), d, i)
                for p, d, i in datasets_by_priority
            )
            ...
            for priority_level in sorted(priorities):
                ...
                for p, ds, ipc in datasets_by_priority:
                    if p == priority_level:  # same priority
                        extend_dol(
                            priority_images_perclass, ipc
                        )  # TODO update clobbers previous lists. this is no bueno. we want to EXTEND any existing values
                for key in priority_images_perclass:
                    random.shuffle(priority_images_perclass[key])
                extend_dol(
                    images_perclass, priority_images_perclass
                )  # TODO update clobbers previous lists. this is no bueno. we want to EXTEND any existing values
    ...
    def split(self, ratio1, ratio2, seed=None, minimum_images_per_class="scale"):
        assert ratio1 + ratio2 == 100, (
            "ratio1:ratio2 must sum to 100, instead got {}:{} (total: {})".format(
        ...
        dataset1 = NeustonDataset(
            src=self.src, images_perclass=d1_perclass, transforms=self.transforms
        )
        dataset2 = NeustonDataset(
            src=self.src, images_perclass=d2_perclass, transforms=self.transforms
        )
        assert dataset1.classes == dataset2.classes, (
            "d1-d2_classes:{}, d2-d1_classes:{}".format(
                set(dataset1.classes) - set(dataset2.classes),
                set(dataset2.classes) - set(dataset1.classes),
            )
        )  # possibly fails due to edge case thresholding?
        assert len(dataset1) + len(dataset2) == len(self), (
            "d1_len:{}, d2_len:{}".format(len(dataset1), len(dataset2))
        )  # make sure we don't lose any images somewhere
    ...
    def from_csv(
        ...
        if missing_classes_src:
            ...
            msg = msg.format(
                len(missing_classes_src),
                len(default_images_perclass.keys()),
                src,
        ...
        return cls(
            src=src,
            images_perclass=new_images_perclass,
...
def get_trainval_datasets(args):
    ...
    if ci_eval:
        ...
        msg = msg.format(
            len(ci_eval), len(validation_dataset.classes + ci_eval), args.class_min
        )
...
def get_trainval_transforms(args):
    ...
    if args.flip:
        ...
        if "+V" in args.flip:
            aug_tforms_validation.extend(flip_tforms)
...
class ImageDataset(Dataset):
    ...
    def __init__(self, image_paths, resize=244, input_src=None):
        ...
        self.image_paths = [
            img for img in image_paths if img.endswith(datasets.folder.IMG_EXTENSIONS)
        ]
        ...
        if len(self.image_paths) < len(image_paths):
            print(
                "{} non-image files were ommited".format(
                    len(image_paths) - len(self.image_paths)
                )
            )
