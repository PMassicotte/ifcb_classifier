...
def get_namebrand_model(model_name, num_o_classes, pretrained=False):
    if model_name == "inception_v3":
        model = MODEL_MODULE.inception_v3(
            pretrained
        )  # , num_classes=num_o_classes, aux_logits=False)
        ...
        model.classifier[1] = nn.Conv2d(
            512, num_o_classes, kernel_size=(1, 1), stride=(1, 1)
        )
...
class NeustonModel(ptl.LightningModule):
    def __init__(self, hparams):
        ...
        self.model = get_namebrand_model(
            hparams.MODEL, len(hparams.classes), hparams.pretrained
        )
    ...
    def validation_epoch_end(self, steps):
        ...
        if self.current_epoch == 0:
            self.best_val_loss = (
                np.inf
            )  # takes care of any lingering val_loss from sanity checks

        validation_loss = torch.stack(
            [batch["val_batch_loss"] for batch in steps]
        ).sum()
        ...
        outputs = (
            torch.cat([batch["val_outputs"] for batch in steps], dim=0)
            .detach()
            .cpu()
            .numpy()
        )
        ...
        input_classes = (
            torch.cat([batch["val_input_classes"] for batch in steps], dim=0)
            .detach()
            .cpu()
            .numpy()
        )
        input_srcs = [
            item
            for sublist in [batch["val_input_srcs"] for batch in steps]
            for item in sublist
        ]

        f1_weighted = metrics.f1_score(
            input_classes, output_classes, average="weighted"
        )
        ...
        eoe = eoe.format(
            True if self.current_epoch == self.best_epoch else self.best_epoch + 1,
            self.agg_train_loss,
            validation_loss,
            100 * f1_weighted,
            100 * f1_macro,
    ...
    def test_epoch_end(self, steps):
        ...
        for steps, dataset in zip(steps, datasets):
            outputs = (
                torch.cat([batch["test_outputs"] for batch in steps], dim=0)
                .detach()
                .cpu()
                .numpy()
            )
