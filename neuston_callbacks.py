...
class SaveValidationResults(ptl.callbacks.base.Callback):
    ...
    def on_validation_end(self, trainer, pl_module):
        ...
        image_basenames = [
            os.path.splitext(os.path.basename(img))[0] for img in image_fullpaths
        ]

        assert output_scores.shape[0] == len(input_classes), (
            "wrong number inputs-to-outputs"
        )
        assert output_scores.shape[1] == len(class_labels), (
            "wrong number of class labels"
        )
        ...
        for mode in ["weighted", "macro", None]:
            for stat in ["f1", "recall", "precision"]:
                metric = getattr(metrics, stat + "_score")(
                    input_classes,
                    output_classes,
                    labels=class_idxs,
                    average=mode,
        ...
        classes_by = dict()
        classes_by["count"] = sorted(
            class_idxs, key=lambda idx: (counts_perclass[idx]), reverse=True
        )  # higher is better
        for stat in ["f1", "recall", "precision"]:
            classes_by[stat] = sorted(
                class_idxs,
                key=lambda idx: (stats[stat + "_perclass"][idx]),
                reverse=True,
            )
        ...
        confusion_matrix = metrics.confusion_matrix(
            input_classes, output_classes, labels=class_idxs, normalize=None
        )
        ...
        # optional values
        if "image_fullpaths" in self.series:
            results["image_fullpaths"] = image_fullpaths
        if "image_basenames" in self.series:
            results["image_basenames"] = image_basenames
        if "training_image_fullpaths" in self.series:
            results["training_image_fullpaths"] = training_image_fullpaths
        if "training_image_basenames" in self.series:
            results["training_image_basenames"] = training_image_basenames
        if "training_classes" in self.series:
            results["training_classes"] = training_classes
        if "output_winscores" in self.series:
            results["output_winscores"] = output_winscores
        if "output_scores" in self.series:
            results["output_scores"] = output_scores
        if "confusion_matrix" in self.series:
            results["confusion_matrix"] = confusion_matrix
            # results['classes_by_recall'] = classes_by['recall'] # no longer included with cm
        if "counts_perclass" in self.series:
            results["counts_perclass"] = counts_perclass
        if "val_counts_perclass" in self.series:
            results["val_counts_perclass"] = val_counts_perclass
        if "train_counts_perclass" in self.series:
            results["val_counts_perclass"] = val_counts_perclass

        # optional stats and class_by's
        for stat in stats:  # eg f1_weighted, recall_perclass
            if stat in self.series:
                results[stat] = stats[stat]
        for stat in classes_by:
            classes_by_stat = "classes_by_" + stat
            if classes_by_stat in self.series:
                results[classes_by_stat] = classes_by[stat]
    ...
    def save_validation_results(self, outfile, results):
        if outfile.endswith(".json"):
            self._save_validation_results_json(outfile, results)
        if outfile.endswith(".mat"):
            self._save_validation_results_mat(outfile, results)
        if outfile.endswith(".h5"):
            self._save_validation_results_hdf(outfile, results)
    ...
    def _save_validation_results_mat(self, outfile, results):
        ...
        idx_data += [
            "classes_by_" + stat for stat in "f1 recall precision count".split()
        ]
        str_data = [
            "class_labels",
            "image_fullpaths",
        ...
        for series in results:
            if isinstance(results[series], np.ndarray):
                results[series] = results[series].astype("f4")
            elif isinstance(results[series], np.float64):
                results[series] = results[series].astype("f4")
            elif series in str_data:
                results[series] = np.asarray(results[series], dtype="object")
            elif series in idx_data:
                results[series] = np.asarray(results[series]).astype("u4") + 1
    ...
    def _save_validation_results_hdf(self, outfile, results):
        ...
        with h5.File(outfile, "w") as f:
            ...
            for series in results:
                if series in attrib_data:
                    meta.attrs[series] = results[series]
                    ...
                    f.create_dataset(
                        series, data=results[series], compression="gzip", dtype="int16"
                    )
...
def save_run_results(
    ...
    assert output_scores.shape[0] == len(output_classes), (
        "wrong number inputs-to-outputs"
    )
        ...
        if "{INPUT_SUBDIRS}" in outfile:
            ...
            for img_path, img_classidx, img_scores in zip(
                ...
                if parent_dir not in dir_groups:
                    dir_groups[parent_dir] = {
                        k: v
                        if k not in ["input_images", "output_classes", "output_scores"]
                        else []
            ...
            for parent_dir, sub_results in dir_groups.items():
                ...
                sub_results["output_classes"] = np.asarray(
                    sub_results["output_classes"], dtype=results["output_classes"].dtype
                )
                sub_results["output_scores"] = np.asarray(
                    sub_results["output_scores"], dtype=results["output_scores"].dtype
                )
...
def _save_run_results(outfile, results):
    ...
    assert ext in [".json", ".mat", ".h5"], 'output fileformat "{}" not valid'.format(
        ext
    )
    ...
    def _save_run_results_mat(outfile, results):
        ...
        if "bin_id" in results:
            output["bin_id"] = results["bin_id"]
            output["roi_numbers"] = results[
                "roi_numbers"
            ]  # .astype('u4') #not numpy yet, but thats fine it seems
        else:
    ...
    if outfile.endswith(".json"):
        _save_run_results_json(outfile, results)
    if outfile.endswith(".mat"):
        _save_run_results_mat(outfile, results)
    if outfile.endswith(".h5"):
        _save_run_results_hdf(outfile, results)
