...
def do_export(args):
    ...
    if args.output:
        ...
        if args.half:
            output = output.replace(".onnx", ".FP16.onnx")
...
if __name__ == "__main__":
    ...
    subparsers = parser.add_subparsers(
        dest="cmd_mode", help="These sub-commands are mutually exclusive."
    )
    ...
    export.add_argument(
        "--half",
        action="store_true",
        help="Exports model using 16bit floating point precision",
    )
    export.add_argument(
        "--device",
        default="cpu",
        choices=("cpu", "cuda"),
        help='Device to load model and tensors to. Default is "cpu"',
    )
    export.add_argument(
        "--opset", default=12, type=int, help="Opset Version for onnx. Default is 12."
    )
    export.add_argument(
        "--batchsize",
        default=0,
        type=int,
        help="Set a fixed batch input/output batch size for the model. Default is None, ie dynamic batch size",
    )
    export.add_argument(
        "--output",
        default=None,
        help='Same as model file but with ".ptl" replaced with ".onnx"',
    )
